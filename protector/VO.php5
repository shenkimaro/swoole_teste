...
//		$this->fkName[]=$objName;
	...
	public function setVO($res='') {
		...
		foreach ($fields as $key => $val) {
			...
			if((@$this->getFieldRestriction($key)=='notnull')||(@trim($value)!='')||(is_array($value))) {
				if(($this->getFieldRestriction($key)=='notnull')&&(trim($value)=='')) throw new Exception($key." vazio");
