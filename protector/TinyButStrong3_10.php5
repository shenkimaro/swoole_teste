...
if(!class_exists('tbsByPage')){
	require __DIR__.'/tbs/3.10/plugins/tbs_plugin_bypage.php';
}
if(!class_exists('clsTbsPlugInHtml')){
	require __DIR__.'/tbs/3.10/plugins/tbs_plugin_html.php';
}
if(!class_exists('tbsNavBar')){
	require __DIR__.'/tbs/3.10/plugins/tbs_plugin_navbar.php';
}
if(!class_exists('clsTbsThisPlugIn')){
	require __DIR__.'/tbs/3.10/plugins/tbs_plugin_syntaxes.php';
}
if(!class_exists('tbsMergeOnFly')){
	require __DIR__.'/tbs/3.10/plugins/tbs_plugin_mergeonfly.php';
}
if(!class_exists('clsTbsCacheSytem')){
	require __DIR__.'/tbs/3.10/plugins/tbs_plugin_cache.php';
}
...
function __construct($Options=null,$VarPrefix='',$FctPrefix='') {
	...
	if (is_string($Options)) {
		...
		if ($Chrs!=='') {
			...
			if ($Err) $this->meth_Misc_Alert('with TinyButStrong3_10() function','value \''.$Chrs.'\' is a bad tag delimitor definition.');
		}
	} 

	// Set options
	$this->setGlobalsVarRef();	
...
public static function getInstance(){
	...
	{		
		self::$instance = new TinyButStrong3_10();
	}
...
private function setGlobalsVarRef() {
	$this->VarRef = $GLOBALS;
}
...
private function sincronizarGlobalsVarRef() {
	if(PHP_VERSION_ID < 80100){
		return null;
	}
	foreach ($GLOBALS as $key => $value) {
		if(!isset($this->VarRef[$key])){
