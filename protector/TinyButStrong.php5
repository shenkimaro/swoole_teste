...
public $_ChrClose = ']';
public $_ChrVal = '[val]';
public $_ChrProtect = '[';//&#91; user as protect
...
public static function getInstance(){
	...
	{		
		self::$instance = new TinyButStrong();
	}
...
public function PlugIn($Prm1,$Prm2=0) {
	...
	if (is_numeric($Prm1)) {
		...
		case TBS_INSTALL: // Try to install the plug-in
			$PlugInId = $Prm2;
			if($PlugInId == 'clsTbsPlugInHtml'){
				if (!class_exists('clsTbsPlugInHtml')) {
					include './pluginsTBS/plug-ins/tbs_plugin_html.php';
				}
			}
