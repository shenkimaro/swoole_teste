...
class TDG {
	...
	public static function getConnectionDbLog($array = array()) {
		...
		if (in_array($_SERVER['SERVER_ADDR'], array($GLOBALS['configDb']['desenvIp'] ?? '', '127.0.0.1', 'localhost', '::1'))) {
			if (count($array) > 0) {
				...
				$log = Log::getInstance($array['configDb']['host'], $array['configDb']['db'], $array['configDb']['login'], $array['configDb']['password'], $GLOBALS['configLog']['table'], $port);
			} else {
				if (isset($GLOBALS['configLog']['login'])) {
					$log = Log::getInstance($GLOBALS['configDb']['host'], $GLOBALS['configLog']['db'], $GLOBALS['configLog']['login'], $GLOBALS['configLog']['password'], $GLOBALS['configLog']['table']);
				} else {
					$log = Log::getInstance($GLOBALS['configDb']['host'], $GLOBALS['configLog']['db'], $GLOBALS['configDb']['login'], $GLOBALS['configDb']['password'], $GLOBALS['configLog']['table']);
				}
			}
		} else if (in_array($_SERVER['SERVER_ADDR'], $ipsConfig) || $serverName == $serverNameConfig) {
			if (count($array) > 0) {
				$log = Log::getInstance($array['configDb']['_host'], $array['configDb']['_db'], $array['configDb']['_login'], $array['configDb']['_password'], $GLOBALS['configLog']['table']);
			} else {
				if (isset($GLOBALS['configLog']['_login'])) {
					$log = Log::getInstance($GLOBALS['configDb']['_host'], $GLOBALS['configLog']['_db'], $GLOBALS['configLog']['_login'], $GLOBALS['configLog']['_password'], $GLOBALS['configLog']['table']);
				} else {
					$log = Log::getInstance($GLOBALS['configDb']['_host'], $GLOBALS['configLog']['_db'], $GLOBALS['configDb']['_login'], $GLOBALS['configDb']['_password'], $GLOBALS['configLog']['table']);
				}
			}
		}
		if ($log == null) {
	...
	public function buildInsert(DTO $dto, $pk = false, $returnPk = false) {
		...
		foreach ($methods as $method) {
			...
			} else if (strtolower(trim($value)) == DTO::EMPTY_STRING) {
				$sqlValues .= $sqlValues == "" ? "''" : ",''";
			} else {
	...
	public function formatOrder($array = array(), $default_order = '') {
		...
		if (isset($array['_by']) && (strtoupper(trim($array['_by'])) == 'ASC' || strtoupper(trim($array['_by'])) == 'DESC') && isset($array['_order']) && trim($array['_order']) != '')
